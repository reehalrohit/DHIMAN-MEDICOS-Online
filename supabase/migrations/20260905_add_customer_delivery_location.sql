-- Keeps the repository schema in sync with the live Supabase database.
...
alter table public.customer_orders
  drop constraint if exists customer_orders_delivery_distance_km_range;

alter table public.customer_orders
  add constraint customer_orders_delivery_distance_km_range
  check (
    delivery_distance_km is null
    or (delivery_distance_km >= 0 and delivery_distance_km <= 100)
  );

alter table public.customer_orders
  drop constraint if exists customer_orders_delivery_latitude_range;
...
alter table public.customer_orders
  drop constraint if exists customer_orders_delivery_longitude_range;
