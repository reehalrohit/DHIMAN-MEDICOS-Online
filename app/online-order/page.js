...
export default function OnlineOrderPage(){
 ...
 function requestDeliveryLocation(){
  ...
  navigator.geolocation.getCurrentPosition(
    pos=>{setDeliveryLocation({latitude:pos.coords.latitude,longitude:pos.coords.longitude,accuracy:pos.coords.accuracy});setLocating(false);},
 ...
 <>
  ...
  <div style={{padding:12,borderRadius:12,background:"#f2f6f3",border:"1px solid #d7e3db"}}>
    ...
    <button type="button" onClick={requestDeliveryLocation} disabled={locating||placing} style={s.secondary}>
      {locating ? "Checking location…" : deliveryLocation ? "✓ Location verified — check again" : "Check delivery location"}
    </button>
    {total<199 && <div style={{fontSize:12,color:"#a33f34",marginTop:7}}>Add ₹{(199-total).toFixed(2)} more for delivery.</div>}
  </div>
)}
<button onClick={placeOrder} disabled={placing || (form.delivery==="delivery" && (total<199 || !deliveryLocation || form.payment!=="razorpay"))} style={s.primary}>{placing?"Processing…":form.payment==="razorpay"?"Pay & place order":"Place order"}</button></div></div></div>}
 </main>
}
